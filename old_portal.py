    ...
    if is_udp:
        ...
        while flag:
            ...
            if struct.unpack("B", seg[0:1])[0] > 1:
                ...
                if detect:
                    frame = detect_objects(frame)  # Ajout de la détection
...
def video_feed(room_id):
    ...
    if room.stream_address == "local":
        ...
        return Response(get_frame(room.stream_address), mimetype='multipart/x-mixed-replace; boundary=frame')
    else:
        return Response(get_frame(room.stream_address, is_udp=True), mimetype='multipart/x-mixed-replace; boundary=frame')
...
def detect_video(room_id):
    ...
    def generate():
        cap = cv2.VideoCapture(0)  # 0 pour webcam locale, changer si besoin
        while True:
            success, frame = cap.read()
            if not success:
